...
function ObjButtonBuild() {
  ...
  xOffset = parseFloat(xOffset.toFixed(5)); // LD-8814
  yOffset = parseFloat(yOffset.toFixed(5)); // LD-8814
  ...
  this.bIsWCAG = is.bWCAG;
	...
	if(this.hasOuterShadow && is.svg)
	{
		...
		{
			// LD-8814: adjusting width and height for Left, Right, Top and Bottom offset shadow type
			if(xOffset == 0)
			{
			...
			if(xOffset <= 0 && is.svg) // LD-8814: include top and bottom offset shadow type
			{	
				// LD-8814: removed the hardcoded 1px, its no longer needed for TopLeft ((xOffset<0 && yOffset<0)) offset shadow type
				adjustedXPos = this.x  + (1 * (xOffset - this.outerShadowBlurRadius));
			}			
			if(yOffset <= 0 && is.svg) //LD-8814: include left and right offset shadow type
			{	
	...
	// LD-8814 CHANGE: Removed +3px width growth to prevent unintended layout widening
	// that contributed to button misalignment with shadows. 
	//if (xOffset != 0 || yOffset != 0) 
	//	if(!this.name.indexOf("button")>-1)
	//		adjustedWidth +=3;
...
function ObjButtonBuildSvg(){
	...
	{			
		...
		{	
			...
			if(xOffset <= 0) // LD-8814: include top and bottom offset shadow type
			{
			...
			if(yOffset <= 0) // LD-8814: include left andright offset shadow type
			{
...
function ObjButtonBuildSpanText(name, mapOffsetX, mapOffsetY)
{
	...
	{
		// LD-8706: Adjusted text positioning based on the object's width and height
		// LD-8814: Note incase the bounds 1px from shapeutils.cpp file, GetShapeBounds function is modified 
		// the same change should be updated here
		txtReturn += "left:"+ mapOffsetX + "px; top:" +mapOffsetY + "px; width:" + (this.w-1) + "px; height:" + (this.h-1) + "px;";
	}
	txtReturn += '"';
...
function ObjButtonUp(e) {
  ...
  if( !is.ieMac && !is.nsMac && e && e.button==2 )
  {
    if( this.hasOnRUp )
	{
	  // LD-8767: get the original context menu handler before overriding it so it can be restored
      ocmOrig = getDisplayDocument().oncontextmenu;
...
function ObjSVGShadow(objWidth, objHeight, xOffset, yOffset, thisObj)
{
	...
	if(xOffset <= 0 || yOffset <= 0) // LD-8706: xOffset and yOffset value is 0 for center offset shadow 
	{
		...
		if(yOffset <= 0) // LD-8814: include left and right offset shadow type
	...
	if(xOffset <= 0 || yOffset <= 0) // LD-8814: include top, bottom, left and right offset shadow type
	{	
...
function ObjRespChanges()
{
	...
	// LD-8762 Update rotation angle and flip
	var divTag = getHTMLEleByID(this.name);
	divTag.style.transform = 'rotate(' + this.r + 'deg) scaleX(' + (this.hf ? '-1' : '1') + ') scaleY(' + (this.vf ? '-1' : '1') + ')';
}
...
function ObjButtonGetCSS(){
	...
	// LD-8814: Removed +3px width growth to prevent unintended layout widening
	// that contributed to button misalignment with shadows. 
	//if (xOffset !== 0 || yOffset !== 0)
	//	if(!this.name.indexOf("button")>-1)
			//adjustedWidth +=3;
