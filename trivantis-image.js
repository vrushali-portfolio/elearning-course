...
function ObjImageBuildSvg(){
	...
	{	
		...
		{			
			if (xOffset == 0) // LD-8829: adjusting width and height for Top and Bottom offset shadow type
			{
				adjustedWidth = this.w + (1 * Math.abs(xOffset)) + (2 * this.borderWeight) + (2 * this.outerShadowBlurRadius);				
				adjustedHeight = this.h + (1 * Math.abs(yOffset)) + (2 * this.borderWeight) + this.outerShadowBlurRadius;
			}
			else if (yOffset == 0) // LD-8829: adjusting width and height for Left and Right offset shadow type
			{
				adjustedWidth = this.w + (1 * Math.abs(xOffset)) + (2 * this.borderWeight) + this.outerShadowBlurRadius;
				adjustedHeight = this.h + (1 * Math.abs(yOffset)) + (2 * this.borderWeight) + (2*this.outerShadowBlurRadius);
			}
			else
			{
				adjustedWidth = this.w + (1 * Math.abs(xOffset)) + (2 * this.borderWeight) + this.outerShadowBlurRadius;
	...
	if(this.hasOuterShadow)
	{
		...
		{	
			...
			if (xOffset <= 0) // LD-8829: includes top, bottom left and right offset shadow type
			{
			...
			if (yOffset <= 0) // LD-8829: includes top, right, left offset shadow type
			{
...
function ObjShapeBuildSvg(){
	...
	if(this.hasOuterShadow)
	{
		...
		else
		{
			if (xOffset == 0) // LD-8829: adjusting width and height for top and bottom offset shadow type
			{
				adjustedWidth = Math.ceil(this.w + (1 * Math.abs(xOffset)) + (2 * this.outerShadowBlurRadius));
			...
			else if (yOffset == 0) // LD-8829: adjusting width and height for left and right offset shadow type
			{
				...
				adjustedHeight = Math.ceil(this.h + (1 * Math.abs(yOffset)) + (2 * this.outerShadowBlurRadius)); 
			}
			else
			{
				adjustedWidth = Math.ceil(this.w + (1 * Math.abs(xOffset))  + this.outerShadowBlurRadius);
	...
	if(  is.svg && ( !UseHtmlImgTag(this)  || this.bUseSvgFile || bHasEffects))
	{
		...
		else
		{
			...
			else if(xOffset < 0 || yOffset < 0)
			{
				...
				{
					adjustedXPos += (xOffset - this.outerShadowBlurRadius);
					if (yOffset == 0) adjustedYPos += (yOffset - this.outerShadowBlurRadius); // LD-8829: left offset shadow type
					clipRect = 'clip: rect(' + 0 + 'px ' + adjustedWidth + 'px ' + adjustedHeight + 'px ' + (xOffset - this.outerShadowBlurRadius) + 'px);';
				}
				else if(xOffset >= 0 && yOffset < 0)
				{
					if (xOffset == 0) adjustedXPos += (xOffset - this.outerShadowBlurRadius); // LD-8829: top offset shadow type
					adjustedYPos += (yOffset - this.outerShadowBlurRadius);
			...
			{
				if(xOffset == 0) // LD-8829: for bottom offset shadow type
				{
					adjustedXPos += (xOffset - this.outerShadowBlurRadius);
					clipRect = 'clip: rect(' + 0 + 'px ' + adjustedWidth + 'px ' + adjustedHeight + 'px ' + (xOffset - this.outerShadowBlurRadius) + 'px);';
				}
				else if(yOffset == 0) // LD-8829: for right offset shadow type
				{
					adjustedYPos += (yOffset - this.outerShadowBlurRadius);
					clipRect = 'clip: rect(' + (yOffset - this.outerShadowBlurRadius) + 'px ' + adjustedWidth + 'px ' + adjustedHeight + 'px ' + 0 + 'px);';
				}
