...
function addRotateCSS(angle, hasShadow, width, height, xPos, yPos, shadowDirection, shadowDepth, shadowBlurRadius, verticalFlip, horizontalFlip, boundsRectX, boundsRectY, adornerWidth, adornerHeight)
{
	...
	if (is.chrome || is.safari)
	{
		...
		if (typeof angle == 'number')
			rotateAttribute += 'rotate(' + angle + 'deg) ';
...
function addClickMap(objWidth, objHeight, xOffset, yOffset, thisObj)
{
	...
	// LD-8829: includes map offset adjustment for center, left, right, top and bottom offset shadow type
 ...
 * LD-8767: getDivObj
 * ---------
 * Finds the object in the global arObjs array that is associated with a given DOM element.
 * This is used to determine whether a click target belongs to an interactive object
 * so that page-level click actions don't trigger on object clicks.
 *
 * @param {HTMLElement} div - The DOM element that was clicked or targeted.
 * @returns {Object|null} - The matching object from arObjs or null if not found.
 */
function getDivObj(div)
{
    var obj = null;
    var arObjs = window.arObjs || []; // List of all interactive objects

    // Traverse upward through the DOM until a match is found or no parent remains
    while (!obj && div)
    {
        // Compare against every known object in arObjs
        for (var i = 0; i < arObjs.length; i++)
        {
            // Direct reference match
            if (arObjs[i].div && arObjs[i].div === div)
            {
                obj = arObjs[i];
                break;
            }
            // ID match as fallback
            if (arObjs[i].div && arObjs[i].div.id && div.id && arObjs[i].div.id === div.id)
            {
                obj = arObjs[i];
                break;
            }
...
/**
 * LD-8767: isLeftClickEvent
 * ----------------
 * Checks whether an event represents a left-click or keyboard equivalent
 * (Space or Enter).
 *
 * @param {Event} e - The event to check.
 * @returns {boolean} - True if it's a left-click or keyboard equivalent.
 */
function isLeftClickEvent(e) 
{
    return (e.button === 0 || e.which === 1) || // Mouse left button
           (!e.which && (e.button === 0 || e.button === 1)); // Edge cases
}

/**
 * LD-8767: processPageMouseAct
 * -------------------
 * Handles page-level mouse events (left-click, right-click).
 * This only triggers if the click is NOT on an interactive object (obj)
 * and is inside the main page container (pageDIV or fixDIV).
 * Left-click will trigger pageLClick(), right-click will trigger pageRClick()
 * while also preventing the browser's default context menu.
 *
 * @param {MouseEvent} e - The mouse or contextmenu event.
 * @returns {boolean} - Always true unless prevented for right-click.
 */
function processPageMouseAct(e) 
{   
    var rc = true;      

    // Optional: Check for scrolling logic that might block clicks
    if (typeof ObjScrollTestActive === 'function' && ObjScrollTestActive())
        return false;
    ...
    var obj = getDivObj(div); // Find if the click is on an object
    var isLeftClick = isLeftClickEvent(e);  
    var pageDiv = GetCurrentPageDiv();  
    var fixDiv = getDisplayDocument().getElementById('fixDIV');

    // Page-level click = anywhere inside pageDiv or fixDiv that is NOT an object
    var isPageLevelClick = (pageDiv && pageDiv.contains(div)) || 
                           (fixDiv && fixDiv.contains(div));  

    // Only trigger if it's NOT an object click and inside the page-level container
    if (!obj && isPageLevelClick)
    {
        if (isLeftClick && typeof pageLClick === 'function') 
        {
            pageLClick(e);
            cancelEvent(e);
        } 
        else if (e.type === 'contextmenu' && typeof pageRClick === 'function') 
        {
            pageRClick(e);
			cancelEvent(e);
        }       
...
/**
 * LD-8767: cancelEvent - Copied from LO
 * -------------------
...
// LD-8767: Attach mousedown and contextmenu event listeners
getDisplayDocument().addEventListener("mousedown", processPageMouseAct);
getDisplayDocument().addEventListener("contextmenu", processPageMouseAct);
...
// LD-8243: get iOS version
function getiOSVersion() 
{
	var match = navigator.userAgent.match(/OS (\d+)_?(\d+)?/);
	if (match && /iPhone|iPad|iPod/.test(navigator.userAgent)) 
	{
...
function isSppInIosFrame()
{
	...
	if (is.isMobile.iOS())
	{
		...
		// LD-8243 - Allow fallback for iOS 13.3 and below
		// Check if the user is on iOS and using a version that requires fallback
		// Previously, we also checked for isSinglePagePlayerAvail(), but that was removed
		// because on older iOS versions, SPP wasn't working correctly
		// Removing that check ensures fallback works even when SPP is off

		var allowFallback = iOSVersion &&
			(iOSVersion.major < 13 || (iOSVersion.major === 13 && iOSVersion.minor <= 3));

		result = allowFallback ? (isInIframe(getDisplayWindow(), 0) || isInFrame(getDisplayWindow(), 0)) : false;
	}
...
// LD-8274: To fix the questions reset issue - if reset action added on the pageshow 
window.addEventListener('pageshow', function (event) 
{  
  if (event.persisted) 
  {
	// SPP OFF (Local file system -> Firefox, Local server -> Chrome & Firefox, http/https server -> Firefox)
	// If the page was restored from BFCache (browser's back-forward cache)
    // Scripts won't re-run, so force a reload to re-trigger everything	
    location.reload();
  } 
  else if (!isSppCapable() || (typeof(bTrivRunView) != 'undefined' && bTrivRunView)) 
  {	 
	// SPP ON/OFF (File System Launch -> Chrome browser), and Run/preview/Debug mode (runs as non-SPP)
	// This scenario will reload the page, but the form values(radio, checkbox, dropdown, etc) will be retained after the script runs
	// Only form values will be retained, not the variables
	// So, updating the form values based on the value stored in the question variables	
    // Check if questions exist (e.g., a quiz page)
	
    try 
	{
        if (typeof trivQuestionArray !== 'undefined' && trivQuestionArray.length > 0) 
        {
          trivQuestionArray.forEach(function (questionObj) 
          {
            // Get the question's name (like "qu45")
            var questionName = questionObj.name;
            var updateFuncName = "Update_" + questionName;
            
            // Call the corresponding Update function to sync UI with restored values
            if (typeof window[updateFuncName] === "function") 
            {
              window[updateFuncName](); // Sync each question's UI  Rank seq, MC, MR, TF, hotspot, Likert, no need for DD, still calling will not break anything
            }
            else if(typeof trivResetEntry === "function")
            {
              trivResetEntry(""); // Fill in the blank,  Number Entry, essay, short answer
            }
            // Match, DD (it has updateFuncName) are not form related
